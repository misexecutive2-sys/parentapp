...
export default function Attendance() {
  ...
  // ── On mount: fetch classes and subjects only ──
  ...
  const fetchClasses = async () => {
    try {
      ...
      else if (Array.isArray(d.class)) classList = d.class;       // API returns "class": [...]
      else if (Array.isArray(d.classes)) classList = d.classes;
      else if (Array.isArray(d.data)) classList = d.data;

      // Sort by class_order so LKG → UKG → 1 → 2 ... → 10
  ...
  // ── Fetch sections for a given class_id ──
...
const fetchSubjects = async () => {
  try {
    ...
    const r = await fetch(`${API_URL}/api/sched-subjects/${class_id}`, { // ✅ dynamic ID
      headers: { Authorization: `Bearer ${token}` },
    });
    const raw = await r.text();
    if (raw.startsWith('<')) return;
  ...
  // ── Handle class selection (Daily/Session tabs) ──
  ...
  // ── Handle class selection (Monthly tab) ──
    ...
    <View style={[styles.filterCard, { zIndex: 100 }]}>
      ...
      <View style={{ zIndex: 30, marginTop: 10 }}>
        <Dropdown
          ...
          onToggle={() => { const n = !subjectDropOpen; closeAllDropdowns(); setSubjectDropOpen(n); }}
          options={subjects}
