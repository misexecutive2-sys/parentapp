...
// ── Mock Data — remove when /api/app/student-fees is ready ───
const MOCK_FEES = [
  {
    id: 1,
    type: "Tuition Fee",
...
export default function FeesScreen() {
  ...
  useEffect(() => { loadChildAndFees(); }, []);

  // ── Load child info + fees ───────────────────────────────
  const loadChildAndFees = async () => {
    try {
      ...
      // const res = await fetch(`${API_BASE}/api/app/student-fees`, {
      //   method: "POST",
      //   headers: {
  ...
  //
  //  STEP 1 → POST /api/app/create-payment-order  (backend creates Razorpay order)
  //  STEP 2 → Open Razorpay native checkout sheet
  //  STEP 3 → POST /api/app/verify-payment        (backend verifies HMAC signature)
  //  STEP 4 → Update local fee state to "paid"
  //
  const handlePayNow = async (fee: any) => {
    try {
      ...
      const orderRes = await fetch(`${API_BASE}/api/app/create-payment-order`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          fee_id: fee.id,
          amount: fee.amount,       // ₹ — backend converts to paise
      ...
      const options = {
        ...
        currency: "INR",
        key: order.key,                        // from backend (your test/live key)
        amount: String(order.amount),          // in paise, as string
        order_id: order.order_id,              // rzp_order_XXXXXX from backend
        name: "SchoolAid",
        prefill: {
          contact: "",   // optionally prefill parent's phone
          email:   "",   // optionally prefill parent's email
        },
        theme: { color: theme.primary },
      };

      // RazorpayCheckout.open resolves on success, rejects on failure/cancel
      const paymentData = await RazorpayCheckout.open(options);
      // paymentData = { razorpay_payment_id, razorpay_order_id, razorpay_signature }

      // ── STEP 3: Verify payment signature on backend ──────
      const verifyRes = await fetch(`${API_BASE}/api/app/verify-payment`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      Alert.alert(
        "Payment Successful 🎉",
        `₹${fee.amount.toLocaleString()} paid for ${fee.type}.\n\nTransaction ID: ${paymentData.razorpay_payment_id}`
      );

    } catch (err: any) {
      // Razorpay throws { code, description } on cancel/failure
      const message =
        err?.description ?? err?.message ?? "Something went wrong. Please try again.";

      // Don't show error alert if user simply dismissed / cancelled
      if (err?.code === 0 || message.toLowerCase().includes("cancel")) {
        return;
      }
          ...
          {overdueCount > 0 && (
            <View style={styles.overdueWarning}>
              <Text style={styles.overdueWarningText}>
                ⚠️ You have {overdueCount} overdue fee
                {overdueCount > 1 ? "s" : ""}. Please pay immediately to avoid
                penalties.
              </Text>
            </View>
          )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={[styles.statusBadge, { backgroundColor: statusBg(selectedFee?.status ?? "") }]}>
                <Text style={[styles.statusText, { color: statusColor(selectedFee?.status ?? "") }]}>
            ...
            {selectedFee?.status !== "paid" ? (
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  Alert.alert("Receipt", `Transaction ID: ${selectedFee?.transaction_id}`)
                }
                activeOpacity={0.85}
              >
...
const styles = StyleSheet.create({
  ...
  overdueWarning:     { marginHorizontal: 16, marginBottom: 8, padding: 12, backgroundColor: "#fee2e2", borderRadius: 10, borderWidth: 1, borderColor: "#fca5a5" },
  ...
  feeCard:       { backgroundColor: "#fff", marginHorizontal: 16, marginBottom: 12, padding: 16, borderRadius: 16, borderWidth: 1, borderColor: "#E0E6F0" },
