...
/**
 * FeesScreen.tsx — Cashfree Payment Gateway (Real Integration)
 *
 * CHANGES FROM PREVIOUS VERSION:
 *  - SimulateGatewayModal REMOVED entirely
 *  - Real Cashfree SDK integrated via CFPaymentGatewayService
 *  - handlePayNow launches Cashfree checkout with payment_session_id
 *  - Cashfree callbacks (onVerify / onError) drive handleVerify
 *  - CASHFREE_ENV constant — switch "SANDBOX" ↔ "PRODUCTION" for go-live
 *  - Graceful fallback alert if SDK is unavailable (web / bare RN without native build)
 *
 * INSTALL (run in your project root):
 *   npm install @cashfreepayments/react-native-cashfree-pg-sdk
 *   # iOS
 *   cd ios && pod install
 *
 * ANDROID — add to android/app/build.gradle dependencies:
 *   implementation 'com.cashfree.pg:api:2.+'
 *
 * DOCS: https://docs.cashfree.com/docs/react-native-integration
 */
...
// ─────────────────────────────────────────────────────────────
// Cashfree SDK import
// If the SDK is not installed yet the code still compiles;
// attempting to pay will show a clear error message.
// ─────────────────────────────────────────────────────────────
let CFPaymentGatewayService: any = null;
let CFEnvironment: any           = null;
let CFSession: any               = null;
let CFDropCheckoutPayment: any   = null;
let CFThemeData: any             = null;
let CFPaymentComponentBuilder: any = null;
let CFDropCheckoutPaymentBuilder: any = null;

try {
  const sdk = require("@cashfreepayments/react-native-cashfree-pg-sdk");
  CFPaymentGatewayService      = sdk.CFPaymentGatewayService;
  CFEnvironment                = sdk.CFEnvironment;
  CFSession                    = sdk.CFSession;
  CFDropCheckoutPayment        = sdk.CFDropCheckoutPayment;
  CFThemeData                  = sdk.CFThemeData;
  CFPaymentComponentBuilder    = sdk.CFPaymentComponentBuilder;
  CFDropCheckoutPaymentBuilder = sdk.CFDropCheckoutPaymentBuilder;
} catch (_) {
  // SDK not installed — handled gracefully at runtime
}

// ─────────────────────────────────────────────────────────────
// ⚙️  CONFIG — change CASHFREE_ENV to "PRODUCTION" for live
// ─────────────────────────────────────────────────────────────
const CASHFREE_ENV: "SANDBOX" | "PRODUCTION" = "SANDBOX";
...
// ════════════════════════════════════════════════════════════
// ⭐ CASHFREE HOOK — replaces SimulateGatewayModal entirely
// ════════════════════════════════════════════════════════════
/**
 * useCashfreePayment
 *
 * Registers Cashfree SDK callbacks once on mount, unregisters on unmount.
 * Call `launchCashfree(order)` to open the checkout sheet.
 * Results are delivered via onSuccess / onError props.
 */
function useCashfreePayment({
  onSuccess,
  onError,
}: {
  onSuccess: (orderId: string, paymentId: string) => void;
  onError:   (orderId: string, errorMsg: string)  => void;
}) {
  // Keep latest callbacks in a ref so the registered listener always sees current values
  const cbRef = useRef({ onSuccess, onError });
  useEffect(() => { cbRef.current = { onSuccess, onError }; });
  ...
  useEffect(() => {
    ...
    CFPaymentGatewayService.setCallback({
      onVerify: (order: any) => {
        // order.getOrderId() — available in some SDK versions; fall back to order directly
        const orderId   = typeof order.getOrderId === "function" ? order.getOrderId()   : (order.orderId   ?? order.order_id   ?? "");
        const paymentId = typeof order.getPaymentId === "function" ? order.getPaymentId() : (order.paymentId ?? order.cf_payment_id ?? "");
        cbRef.current.onSuccess(orderId, paymentId);
      },
      // ── Error / cancel callback ───────────────────────────
      onError: (error: any, order: any) => {
        const orderId = typeof order?.getOrderId === "function" ? order.getOrderId() : (order?.orderId ?? order?.order_id ?? "");
        const msg     = typeof error?.getMessage === "function"
          ? error.getMessage()
          : (error?.message ?? error?.toString() ?? "Payment failed");

        // Cashfree reports user-cancellation as an error; detect common cancel strings
        const isCancelled = /cancel|dismiss|back/i.test(msg);
        cbRef.current.onError(orderId, isCancelled ? "CANCELLED::" + msg : msg);
      },
    });

    return () => {
      // Clean up listener to avoid leaks when navigating away
      try { CFPaymentGatewayService.removeCallback?.(); } catch (_) {}
    };
  }, []);
   ...
   * @param order  — OrderData returned by your create-order API (API 6)
   */
  const launchCashfree = useCallback((order: OrderData) => {
    if (!CFPaymentGatewayService || !CFSession || !CFEnvironment || !CFDropCheckoutPaymentBuilder) {
      Alert.alert(
        "SDK Not Found",
        "The Cashfree SDK is not installed.\n\nRun:\n  npm install @cashfreepayments/react-native-cashfree-pg-sdk\n\nthen rebuild the app.",
        [{ text: "OK" }]
      );
      return;
    }
    ...
    try {
      const env = CASHFREE_ENV === "PRODUCTION"
        ? CFEnvironment.PRODUCTION
        : CFEnvironment.SANDBOX;

      // Build the session object
      const session = new CFSession(
        order.payment_session_id,  // payment_session_id from your backend
        order.order_id,            // order_id
        env
      );

      // Theme — uses your app's primary colour
      const theme = new CFThemeData(
        C.primary,   // navigationBarBackgroundColor
        C.white,     // navigationBarTextColor
        C.primary,   // buttonBackgroundColor
        C.white,     // buttonTextColor
        C.primary    // primaryTextColor
      );

      // Enable all payment components (UPI, cards, netbanking, wallets, EMI, BNPL)
      const paymentComponent = new CFPaymentComponentBuilder()
        .add(CFPaymentComponent.CF_CARD)
        .add(CFPaymentComponent.CF_UPI)
        .add(CFPaymentComponent.CF_NET_BANKING)
        .add(CFPaymentComponent.CF_WALLET)
        .add(CFPaymentComponent.CF_EMI)
        .add(CFPaymentComponent.CF_PAY_LATER)
        .build();

      const dropPayment = new CFDropCheckoutPaymentBuilder()
        .setSession(session)
      ...
      Alert.alert("Payment Error", e?.message ?? "Unable to launch payment gateway.");
    }
  }, []);

  return { launchCashfree };
}

// CFPaymentComponent enum — inline so we don't need an extra import
const CFPaymentComponent = {
  CF_CARD:        "card",
  CF_UPI:         "upi",
  CF_NET_BANKING: "netbanking",
  CF_WALLET:      "wallet",
  CF_EMI:         "emi",
  CF_PAY_LATER:   "paylater",
};
...
function StudentFeesScreen({ student, onBack }: { student: Student; onBack: () => void }) {
  ...
  // Modal states
  const [paymentTypeModal,   setPaymentTypeModal]   = useState(false);
  const [directPartialModal, setDirectPartialModal] = useState(false);
  const [orderModal,         setOrderModal]         = useState<OrderData | null>(null);
  const [resultStatus,       setResultStatus]       = useState<string | null>(null);

  // Tracks the current order being processed (for verify after gateway returns)
  const pendingOrderRef = useRef<OrderData | null>(null);

  // Tracks ledger_ids resolved by preview
  const resolvedLedgerIds = useRef<number[]>([]);

  // ── Cashfree integration ──────────────────────────────────
  const { launchCashfree } = useCashfreePayment({
    onSuccess: (orderId, paymentId) => {
      // Gateway returned success — call your verify API (API 7)
      handleVerify(orderId, paymentId);
    },
    onError: (orderId, errorMsg) => {
      const isCancelled = errorMsg.startsWith("CANCELLED::");
      const oid = orderId || pendingOrderRef.current?.order_id || "";
      if (isCancelled) {
        // User backed out — still verify so backend marks it cancelled
        handleVerify(oid, "", "cancelled");
      } else {
        Alert.alert("Payment Failed", errorMsg.replace(/^CANCELLED::/, ""));
        handleVerify(oid, "", "failed");
      }
    },
  });
  ...
  const callPreviewApi = async (
    paymentType: "ledger_selection" | "direct_amount",
    amount?: number,
    ledgerIds: number[] = []
  ) => {
    setPvLoading(true);
    try {
      ...
      if (paymentType === "direct_amount" && amount !== undefined) body.amount = amount;
      const d = await api("/api/fees/payment/preview", { method: "POST", body: JSON.stringify(body) });
      const data: PreviewData = d.data ?? d.preview ?? d;
      resolvedLedgerIds.current = data.resolved_ledger_ids ?? ledgerIds;
      setPreview(data);
  ...
  // API 6 — create order, then show confirmation modal
  const handleCreateOrder = async () => {
    setPreview(null);
    setPayLoading(true);
    try {
      ...
      const orderData: OrderData = d.data ?? d;
      pendingOrderRef.current = orderData;
      setOrderModal(orderData);
    } catch (e: any) {
      Alert.alert("Order Failed", e.message);
    } finally {
      setPayLoading(false);
  ...
  /**
   * handlePayNow — called when user taps "Proceed to Pay" in OrderCreatedModal.
   * Dismisses the modal and launches the real Cashfree checkout.
   */
  const handlePayNow = (order: OrderData) => {
    setOrderModal(null);
    pendingOrderRef.current = order;
    launchCashfree(order);
  };

  // API 7 — verify payment result
  const handleVerify = async (orderId: string, paymentId: string, statusOverride?: string) => {
    try {
      let finalStatus: string;
      if (statusOverride) {
        // cancelled or failed — optionally still hit verify so backend records it
        if (statusOverride !== "cancelled") {
          try {
            ...
            finalStatus = d.data?.transaction_status ?? d.transaction_status ?? statusOverride;
          } catch {
            finalStatus = statusOverride;
          }
        } else {
      ...
      // Even if verify fails, show the result screen with "pending"
  ...
  return (
    <View style={sf.root}>
            ...
            >
              {pending.length > 0 ? (
                <>
                  ...
                  {/* Pay Any Amount — no selection required */}
                  <TouchableOpacity
                    style={sf.directPartialBtn}
                    onPress={() => setDirectPartialModal(true)}
                    activeOpacity={0.85}
      ...
      {/* Order confirmation — now calls handlePayNow which triggers real gateway */}
      {orderModal && (
        <OrderCreatedModal
          order={orderModal}
          onCancel={() => {
          ...
          onPayNow={() => handlePayNow(orderModal)}
        />
      )}
