...
// // ════════════════════════════════════════════════════════════
// // SMART SUMMARY CARD — fetches attendance + leaves + homework
// // ════════════════════════════════════════════════════════════
...
// function SmartSummaryCard({ child }: { child: any }) {
...
//       const [attRes, leaveRes, hwRes] = await Promise.allSettled([
//         fetch(
//           `${BASE_URL}/api/attendance/monthly/${child.id}?month=${month}&year=${year}`,
//           { headers }
//         ),
//         fetch(
//           `${BASE_URL}/api/leaves/student?student_id=${child.id}`,
//           { headers }
//         ),
//         fetch(
//           `${BASE_URL}/api/homework/my-submissions?student_id=${child.id}`,
//           { headers }
//         ),
//       ]);
...
//       let homeworkTotal = 0;
//       let homeworkPct: number | null = null;

//       if (hwRes.status === "fulfilled") {
//         const hwData = await hwRes.value.json();
//         const submissions: any[] = hwData.submissions ?? [];
//         const active = submissions.filter((s) => !s.is_cancelled);
//         homeworkTotal = active.length;
//         homeworkOnTime = active.filter((s) => {
...
//         homeworkPct =
//           homeworkTotal > 0
//             ? Math.round((homeworkOnTime / homeworkTotal) * 100)
//             : null;
//       }

//       // ── Build AI summary line locally ──
//       const aiLine = buildAiLine({
//         attendancePct,
//         pendingLeaves,
//         homeworkPct,
//         homeworkTotal,
...
//       );
//     } catch (err) {
//       console.error("SmartSummaryCard error:", err);
//     } finally {
//       setLoading(false);
//     }
//   };

//   // ── Local AI line builder ──
//   const buildAiLine = ({
//     attendancePct,
//     pendingLeaves,
//     homeworkPct,
...
//       if (attendancePct >= 90)
//         parts.push(`${firstName}'s attendance is excellent at ${attendancePct}% this month`);
//       else if (attendancePct >= 75)
//         parts.push(`${firstName} has ${attendancePct}% attendance this month — needs improvement`);
//       else
//         parts.push(`⚠️ Attendance is low at ${attendancePct}% this month — please check`);
//     }

//     if (homeworkPct !== null && homeworkTotal > 0) {
//       if (homeworkPct === 100)
//         parts.push(`all ${homeworkTotal} homework submissions were on time`);
//       else if (homeworkPct >= 75)
//         parts.push(`${homeworkPct}% homework submitted on time`);
//       else
//         parts.push(`only ${homeworkPct}% homework submitted on time — needs attention`);
//     }

//     if (pendingLeaves > 0)
//       parts.push(`${pendingLeaves} leave request${pendingLeaves > 1 ? "s" : ""} pending approval`);
...
//   // ── Attendance color ──
//   const attColor = (pct: number | null) => {
...
//             <View style={sc.aiDot} />
//             <Text style={sc.title}>Smart Summary</Text>
//           </View>
//         </View>
//         <View style={sc.loadingBox}>
//           <ActivityIndicator size="small" color="#0047AB" />
//           <Text style={sc.loadingTxt}>Analysing your child's data…</Text>
//         </View>
//       </View>
//     );
//   }
...
//           {lastUpdated ? (
//             <Text style={sc.updatedTxt}>{lastUpdated}</Text>
//           ) : null}
//           <Ionicons
...
//       {/* AI insight line */}
//       <View style={sc.aiBox}>
//         <Text style={sc.aiIcon}>✨</Text>
//         <Text style={sc.aiText}>{summary.aiLine}</Text>
//       </View>
...
//             {/* Attendance */}
//             <View style={sc.chip}>
//               <View style={[sc.chipIcon, { backgroundColor: attColor(summary.attendancePct) + "18" }]}>
//                 <Ionicons name="calendar-outline" size={16} color={attColor(summary.attendancePct)} />
//               </View>
//               <Text style={[sc.chipValue, { color: attColor(summary.attendancePct) }]}>
//                 {summary.attendancePct !== null ? `${summary.attendancePct}%` : "—"}
//               </Text>
//               <Text style={sc.chipLabel}>Attendance</Text>
//               <Text style={sc.chipSub}>
//                 {summary.presentDays}P · {summary.absentDays}A
//               </Text>
//             </View>

//             {/* Homework */}
//             <View style={sc.chip}>
//               <View style={[sc.chipIcon, { backgroundColor: hwColor(summary.homeworkPct) + "18" }]}>
//                 <Ionicons name="book-outline" size={16} color={hwColor(summary.homeworkPct)} />
//               </View>
//               <Text style={[sc.chipValue, { color: hwColor(summary.homeworkPct) }]}>
//                 {summary.homeworkPct !== null ? `${summary.homeworkPct}%` : "—"}
//               </Text>
//               <Text style={sc.chipLabel}>Homework</Text>
//               <Text style={sc.chipSub}>
//                 {summary.homeworkOnTime}/{summary.homeworkTotal} on time
//               </Text>
//             </View>

//             {/* Leaves */}
//             <View style={sc.chip}>
//               <View style={[sc.chipIcon, { backgroundColor: summary.pendingLeaves > 0 ? "#d9770618" : "#16a34a18" }]}>
//                 <Ionicons
//                   name="time-outline"
//                   size={16}
//                   color={summary.pendingLeaves > 0 ? "#d97706" : "#16a34a"}
//                 />
//               </View>
//               <Text style={[sc.chipValue, { color: summary.pendingLeaves > 0 ? "#d97706" : "#16a34a" }]}>
//                 {summary.pendingLeaves}
//               </Text>
//               <Text style={sc.chipLabel}>Leaves</Text>
//               <Text style={sc.chipSub}>pending</Text>
//             </View>
//           </View>
...
//             <ProgressRow
//               label="Attendance"
//               pct={summary.attendancePct ?? 0}
//               color={attColor(summary.attendancePct)}
//               benchmark={75}
//             />
//             <ProgressRow
//               label="Homework on time"
//               pct={summary.homeworkPct ?? 0}
//               color={hwColor(summary.homeworkPct)}
//               benchmark={80}
//             />
//           </View>

//           {/* Refresh */}
//           <TouchableOpacity style={sc.refreshBtn} onPress={fetchSummary}>
//             <Ionicons name="refresh-outline" size={13} color="#0047AB" />
//             <Text style={sc.refreshTxt}>Refresh Summary</Text>
//           </TouchableOpacity>
//         </>
//       )}
//     </View>
...
//         <Text style={sc.progressLabel}>{label}</Text>
//         <Text style={[sc.progressPct, { color }]}>{pct}%</Text>
//       </View>
//       <View style={sc.progressBg}>
//         <View style={[sc.progressFill, { width: `${Math.min(pct, 100)}%`, backgroundColor: color }]} />
//         {/* Benchmark marker */}
//         <View style={[sc.benchmarkLine, { left: benchmarkLeft }]} />
//       </View>
//       <Text style={sc.benchmarkLabel}>Min. {benchmark}%</Text>
//     </View>
//   );
// }

// // ── SmartSummaryCard styles ──
...
//     paddingVertical: 2,
//   },
//   liveTxt: { fontSize: 9, fontWeight: "800", color: "#16a34a", letterSpacing: 0.5 },
//   headerRight: { flexDirection: "row", alignItems: "center", gap: 8 },
//   updatedTxt: { fontSize: 10, color: "#aaa" },
...
//     marginBottom: 14,
//   },
//   aiIcon: { fontSize: 16, marginTop: 1 },
//   aiText: { flex: 1, fontSize: 13, color: "#1A1A2E", lineHeight: 20, fontWeight: "500" },

//   // Loading
//   loadingBox: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 8 },
//   loadingTxt: { fontSize: 13, color: "#888" },
...
//   chipLabel: { fontSize: 10, fontWeight: "700", color: "#555", textTransform: "uppercase" },
//   chipSub: { fontSize: 9, color: "#aaa", textAlign: "center" },

//   // Progress
//   progressSection: { gap: 12, marginBottom: 14 },
//   progressRow: { gap: 4 },
//   progressLabelRow: { flexDirection: "row", justifyContent: "space-between" },
//   progressLabel: { fontSize: 11, fontWeight: "600", color: "#555" },
...
//     borderRadius: 1,
//   },
//   benchmarkLabel: { fontSize: 9, color: "#aaa", textAlign: "right" },
...
//   },
//   refreshTxt: { fontSize: 12, fontWeight: "600", color: "#0047AB" },
// });
...
// ─────────────────────────────────────────────────────────────
// Smart Summary Card — attendance + leaves + homework
// ─────────────────────────────────────────────────────────────
interface SummaryData {
  attendancePct: number | null;
  presentDays: number;
...
// Color helpers for summary card
const attColor = (pct: number | null): string => {
  if (pct === null) return "#888";
  if (pct >= 90) return "#16a34a";
  if (pct >= 75) return "#d97706";
  return "#dc2626";
...
const hwColor = (pct: number | null): string => {
  if (pct === null) return "#888";
  if (pct >= 90) return "#16a34a";
  if (pct >= 70) return "#d97706";
  return "#dc2626";
...
// Build a natural-language insight line from the fetched data
const buildAiLine = (params: {
  attendancePct: number | null;
  pendingLeaves: number;
  homeworkPct: number | null;
  homeworkTotal: number;
  ...
  if (attendancePct !== null) {
    if (attendancePct >= 90)
      parts.push(
        `${firstName}'s attendance is excellent at ${attendancePct}% this month`,
      );
    else if (attendancePct >= 75)
      parts.push(
        `${firstName} has ${attendancePct}% attendance this month — needs improvement`,
      );
    else
      parts.push(
        `⚠️ Attendance is low at ${attendancePct}% this month — please check`,
      );
  }
  ...
  if (homeworkPct !== null && homeworkTotal > 0) {
    if (homeworkPct === 100)
      ...
      parts.push(
        `only ${homeworkPct}% homework submitted on time — needs attention`,
      );
  }
  ...
  if (parts.length === 0) return "No data available for this month yet.";
  if (parts.length === 1) return parts[0] + ".";
  if (parts.length === 2) return `${parts[0]}, and ${parts[1]}.`;
  return `${parts.slice(0, -1).join(", ")}, and ${parts[parts.length - 1]}.`;
};
...
function ProgressRow({
  ...
  // Use pixel values to avoid TypeScript DimensionValue errors
  const availableWidth = SCREEN_W - 80; // card horizontal padding
  const fillWidth = (Math.min(pct, 100) / 100) * availableWidth;
  const markerLeft = (benchmark / 100) * availableWidth;
  ...
  return (
    <View style={sc.progressRow}>
      ...
      <View style={sc.progressBg}>
        ...
        />
        <View style={[sc.benchmarkLine, { left: markerLeft }]} />
      </View>
      <Text style={sc.benchmarkLabel}>Min. {benchmark}%</Text>
    </View>
  );
...
// Main SmartSummaryCard component
function SmartSummaryCard({ child }: { child: any }) {
  const [summary, setSummary] = useState<SummaryData | null>(null);
  const [loading, setLoading] = useState(true);
  const [collapsed, setCollapsed] = useState(false);
  ...
  const fetchSummary = async () => {
    ...
    try {
      ...
      const [attRes, leaveRes, hwRes] = await Promise.allSettled([
        ...
        fetch(`${BASE_URL}/api/leaves/student?student_id=${child.id}`, {
          headers,
        }),
        fetch(
          `${BASE_URL}/api/homework/my-submissions?student_id=${child.id}`,
  ...
  return (
    <View style={sc.card}>
      ...
      {/* AI insight line — always visible */}
      <View style={sc.aiBox}>
        <Text style={sc.aiIcon}>✨</Text>
        <Text style={sc.aiText}>{summary.aiLine}</Text>
      </View>
          ...
          <View style={sc.chipRow}>
            ...
            <View style={sc.chip}>
              <View
                style={[
                  sc.chipIcon,
                  { backgroundColor: attColor(summary.attendancePct) + "18" },
                ]}
              >
                <Ionicons
            ...
            <View style={sc.chip}>
              <View
                style={[
                  sc.chipIcon,
                  { backgroundColor: hwColor(summary.homeworkPct) + "18" },
                ]}
              >
                <Ionicons
                  name="book-outline"
              ...
              <Text
                style={[sc.chipValue, { color: hwColor(summary.homeworkPct) }]}
              >
                {summary.homeworkPct !== null ? `${summary.homeworkPct}%` : "—"}
              </Text>
              <Text style={sc.chipLabel}>Homework</Text>
            ...
            <View style={sc.chip}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      summary.pendingLeaves > 0 ? "#d9770618" : "#16a34a18",
                  },
                ]}
              >
                <Ionicons
...
export default function InsightScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      {/* ── Content ── */}
      <ScrollView contentContainerStyle={styles.scroll}>
        {/* Smart Summary Card — always at top */}
        {child && <SmartSummaryCard child={child} />}

        {/* Exam picker + analysis button */}
        {loadingExams ? (
          <View style={styles.centered}>
            <ActivityIndicator size="large" color="#0047AB" />
            <Text style={styles.loadingText}>Loading exams…</Text>
...
const sc = StyleSheet.create({
  ...
  titleRow: { flexDirection: "row", alignItems: "center", gap: 8 },
  aiDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: "#0047AB" },
  title: { fontSize: 14, fontWeight: "800", color: "#1A1A2E" },
  livePill: {
    backgroundColor: "#DCFCE7",
    borderRadius: 6,
