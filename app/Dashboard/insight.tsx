...
const SUBJECT_COLORS = [
  "#0047AB", "#16a34a", "#d97706", "#7c3aed", "#db2777", "#0891b2",
];
    ...
    <View style={cStyles.chartBox}>
      ...
      <Svg width={CHART_W} height={chartH}>
        {[0, 25, 50, 75, 100].map((pct) => {
          ...
          return (
            <G key={pct}>
              <Line x1={padL} y1={y} x2={CHART_W - 4} y2={y}
        ...
        {subjects.map((sub, i) => {
          ...
          return (
            <G key={i}>
              <Rect x={x} y={padT} width={barW} height={innerH} rx={6} fill="#EEF2F8" />
              <Rect x={x} y={y} width={barW} height={barH} rx={6} fill={color} opacity={0.88} />
              <SvgText x={x + barW / 2} y={chartH - 8} fontSize={9} fill="#666" textAnchor="middle">
      ...
      <View style={cStyles.legendRow}>
        {subjects.map((sub, i) => (
          <View key={i} style={cStyles.legendItem}>
            <View style={[cStyles.legendDot, { backgroundColor: SUBJECT_COLORS[i % SUBJECT_COLORS.length] }]} />
            <Text style={cStyles.legendText} numberOfLines={1}>{sub.subject_name}</Text>
          </View>
        ))}
      </View>
    </View>
    ...
    <View style={cStyles.chartBox}>
      ...
      <Svg width={CHART_W} height={totalH}>
        {breakdown.map((exam, i) => {
          ...
          return (
            <G key={i}>
              <SvgText x={padL - 8} y={y + barH / 2 + 4} fontSize={10} fill="#444" textAnchor="end" fontWeight="bold">
                {exam.examName}
              </SvgText>
              <Rect x={padL} y={y} width={innerW} height={barH} rx={8} fill="#EEF2F8" />
              <Rect x={padL} y={y} width={Math.max(fillW, 8)} height={barH} rx={8} fill={color} opacity={0.85} />
            </G>
          );
        })}
      </Svg>
  ...
  return (
    <View>
      ...
      <View style={styles.pillRow}>
        ...
        <View style={[styles.pill, { backgroundColor: gradeColor(summary?.overallGrade) + "15" }]}>
          <Text style={[styles.pillValue, { color: gradeColor(summary?.overallGrade) }]}>{summary?.overallGrade}</Text>
          ...
          <View key={idx} style={styles.subjectCard}>
            <View style={styles.subjectRow}>
              ...
              <View style={{ alignItems: "flex-end", gap: 4 }}>
                <View style={[styles.gradePill, { backgroundColor: gradeColor(sub.grade) + "18" }]}>
            ...
            {/* Combined parts — show only % and grade */}
            {sub.is_combined && sub.child_subjects?.length > 0 && (
              <View style={styles.childBox}>
                {sub.child_subjects.map((cs: any, ci: number) => (
                  <View key={ci} style={styles.childRow}>
                    <Text style={styles.childLabel}>Part {ci + 1}</Text>
                    <Text style={styles.childPct}>{parseFloat(cs.percentage).toFixed(1)}%</Text>
                    <View style={[styles.gradePillSm, { backgroundColor: gradeColor(cs.grade) + "15" }]}>
                      <Text style={[styles.gradeTextSm, { color: gradeColor(cs.grade) }]}>{cs.grade}</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const ConsolidatedResults = ({ data }: { data: any }) => {
  ...
  return (
    <View>
      <View style={styles.pillRow}>
        <View style={[styles.pill, { backgroundColor: "#EFF6FF" }]}>
          <Text style={[styles.pillValue, { color: "#0047AB" }]}>{summary?.consolidatedPercentage?.toFixed(1)}%</Text>
        ...
        <View style={[styles.pill, { backgroundColor: "#F0FDF4" }]}>
          <Text style={[styles.pillValue, { color: "#16a34a" }]}>#{summary?.overallRank}</Text>
          <Text style={styles.pillLabel}>Rank</Text>
        </View>
      </View>
        ...
        return (
          <View key={idx} style={styles.subjectCard}>
            <View style={[styles.subjectRow, { marginBottom: 10 }]}>
              <View style={[styles.subjectAccent, { backgroundColor: color }]} />
              <Text style={[styles.subjectName, { flex: 1 }]}>{sub.subject_name}</Text>
            </View>
            <View style={styles.termCompareRow}>
              {(sub.exam_results ?? []).map((res: any, ri: number) => (
                <View key={ri} style={[styles.termChip, { borderColor: color + "40" }]}>
                  <Text style={styles.termChipLabel}>{res.exam_name}</Text>
                  <Text style={[styles.termChipPct, { color }]}>{parseFloat(res.percentage).toFixed(1)}%</Text>
                  <View style={[styles.gradePillSm, { backgroundColor: gradeColor(res.grade) + "15" }]}>
                    <Text style={[styles.gradeTextSm, { color: gradeColor(res.grade) }]}>{res.grade}</Text>
                  </View>
                </View>
              ))}
            </View>
            {sub.exam_results?.length === 2 && (() => {
              const diff = parseFloat(sub.exam_results[1].percentage) - parseFloat(sub.exam_results[0].percentage);
              if (diff > 0.01) return <Text style={[styles.trendText, { color: "#16a34a" }]}>📈 +{diff.toFixed(1)}% improvement</Text>;
              if (diff < -0.01) return <Text style={[styles.trendText, { color: "#dc2626" }]}>📉 {diff.toFixed(1)}% decrease</Text>;
              return <Text style={[styles.trendText, { color: "#888" }]}>➡️ No change</Text>;
            })()}
          </View>
        );
      })}
  ...
  const fetchExams = async () => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/app/student-exams`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "x-academic-year": savedYearId ?? "8" },
...
const fetchAnalysis = async () => {
  ...
  if (!selectedClassId || !selectedSectionId) { Alert.alert("Error", "Class info not loaded yet."); return; }
  try {
    ...
    console.log("🎓 classId:", selectedClassId, "sectionId:", selectedSectionId);

    const url = activeTab === "subject"
      ...
      <View style={styles.tabBar}>
        {(["subject", "consolidated"] as TabType[]).map((tab) => (
          <TouchableOpacity
            ...
            onPress={() => { setActiveTab(tab); setAnalysisData(null); }}
          >
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {loadingExams ? (
          ...
          <>
              ...
              {exams.length > 0 ? (
                <View style={styles.pickerWrapper}>
                  <Picker
                    ...
                    onValueChange={(val) => { setSelectedExamId(val); setAnalysisData(null); }}
                    style={styles.picker}
                    dropdownIconColor="#0047AB"
                  >
                    {exams.map((ex) => (
                      <Picker.Item key={ex.id} label={ex.exam_name ?? ex.name ?? String(ex.id)} value={ex.id} />
                    ))}
                  </Picker>
                </View>
              ) : (
            ...
            <TouchableOpacity
              style={[styles.analysisBtn, (loadingAnalysis || !selectedExamId) && { backgroundColor: "#aaa" }]}
            ...
            >
              {loadingAnalysis ? (
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <Text style={[styles.analysisBtnText, { marginLeft: 8 }]}>Loading…</Text>
                </View>
              ) : (
        ...
        {analysisData ? (
          ...
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptySubtext}>Select an exam and tap View Analysis.</Text>
          </View>
        ) : null}
      </ScrollView>
    </SafeAreaView>
...
const cStyles = StyleSheet.create({
  ...
  legendItem: { flexDirection: "row", alignItems: "center", gap: 4, marginRight: 4, marginBottom: 2 },
...
const styles = StyleSheet.create({
  ...
  iconBtn: { width: 40, height: 40, alignItems: "center", justifyContent: "center", borderRadius: 20, backgroundColor: "rgba(255,255,255,0.15)" },
  iconBtnText: { color: "#fff", fontSize: 20, fontWeight: "600" },
  avatarRow: { flexDirection: "row", alignItems: "center", gap: 10, flex: 1, justifyContent: "center" },
  avatarCircle: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(255,255,255,0.2)", borderWidth: 2, borderColor: "rgba(255,255,255,0.3)" },
  avatarInitials: { color: "#fff", fontSize: 16, fontWeight: "800" },
  headerTitle: { color: "#fff", fontSize: 17, fontWeight: "800" },
  classBadge: { backgroundColor: "rgba(255,255,255,0.2)", paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, marginTop: 3 },
  classBadgeText: { color: "rgba(255,255,255,0.95)", fontSize: 11, fontWeight: "600" },

  tabBar: { flexDirection: "row", backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#E0E6F0" },
  ...
  pickerCard: { backgroundColor: "#fff", borderRadius: 12, borderWidth: 1, borderColor: "#E0E6F0", marginBottom: 12, padding: 12 },
  pickerLabel: { fontSize: 11, fontWeight: "700", color: "#0047AB", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 },
  pickerWrapper: { borderWidth: 1, borderColor: "#E0E6F0", borderRadius: 8, overflow: "hidden", backgroundColor: "#F5F7FA" },
  picker: { height: 50, color: "#1A1A2E" },
  noDataText: { fontSize: 13, color: "#aaa", paddingVertical: 10 },

  analysisBtn: { backgroundColor: "#0047AB", padding: 14, borderRadius: 12, alignItems: "center", marginBottom: 20, marginTop: 4 },
  ...
  pillLabel: { fontSize: 10, color: "#888", marginTop: 2, fontWeight: "600", textTransform: "uppercase" },

  subheading: { fontSize: 12, fontWeight: "800", color: "#666", textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 10, marginTop: 4 },

  subjectCard: { backgroundColor: "#fff", borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, borderColor: "#E0E6F0", elevation: 2 },
  subjectRow: { flexDirection: "row", alignItems: "center", marginBottom: 10, gap: 10 },
  ...
  progressBg: { height: 6, backgroundColor: "#E0E6F0", borderRadius: 3, overflow: "hidden" },
  progressFill: { height: 6, borderRadius: 3 },

  childBox: { backgroundColor: "#F8FAFF", borderRadius: 10, padding: 10, marginTop: 10, gap: 6 },
  ...
  termChip: { flex: 1, minWidth: 100, borderRadius: 10, padding: 10, borderWidth: 1.5, backgroundColor: "#FAFBFF", alignItems: "center" },
  termChipLabel: { fontSize: 10, fontWeight: "700", color: "#666", textTransform: "uppercase", marginBottom: 4 },
  termChipPct: { fontSize: 18, fontWeight: "900", marginBottom: 4 },
  trendText: { fontSize: 12, fontStyle: "italic", marginTop: 8 },

  emptyBox: { alignItems: "center", paddingVertical: 48, backgroundColor: "#fff", borderRadius: 16, borderWidth: 1, borderColor: "#E0E6F0" },
...
const improvStyles = StyleSheet.create({
  ...
  latestPct: { fontSize: 16, fontWeight: "900", minWidth: 42, textAlign: "right" },
});