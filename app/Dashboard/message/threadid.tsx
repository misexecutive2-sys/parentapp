...
const dv = StyleSheet.create({
  ...
  reloadBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.15)", alignItems: "center", justifyContent: "center" },
  title: { flex: 1, fontSize: 14, fontWeight: "700", color: "#fff", textAlign: "center" },
  overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "#1a1a1a", alignItems: "center", justifyContent: "center", zIndex: 10, gap: 12 },
  overlayText: { color: "#fff", fontSize: 13, fontWeight: "600" },
  progressBar: { width: 200, height: 6, backgroundColor: "rgba(255,255,255,0.2)", borderRadius: 3, overflow: "hidden", marginTop: 4 },
  progressFill: { height: "100%" as any, backgroundColor: PRIMARY, borderRadius: 3 },
  retryBtn: { marginTop: 18, backgroundColor: PRIMARY, paddingHorizontal: 28, paddingVertical: 12, borderRadius: 10 },
  retryText: { color: "#fff", fontWeight: "700", fontSize: 14 },
});
...
function InAppDocViewer({ uri, name, onClose }: { uri: string; name: string; onClose: () => void }) {
  ...
  const downloadAndRender = async () => {
    try {
      ...
      // Build full URL — relative paths like "public/uploads/..." need base URL
      ...
      if (fileInfo.exists && (fileInfo.size ?? 0) > 100) {
        ...
        // Send auth token — server returns 401 without it → blank screen
        ...
        // Validate size — a 401 response saves as tiny HTML file
        const info = await FileSystem.getInfoAsync(result.uri) as any;
        if (!info.exists || (info.size ?? 0) < 100) {
          await FileSystem.deleteAsync(result.uri, { idempotent: true });
          throw new Error(`Auth error or empty file (${info.size ?? 0} bytes)`);
        }
      ...
      if (!isImage) {
        ...
        if (!b64 || b64.length < 50) throw new Error("base64 read failed");
        setBase64(b64);
      }
  ...
  // Render PDF inline as base64 — no Google Docs Viewer needed
  const getPdfHtml = () => `<!DOCTYPE html><html><head>
  ...
  const getDocHtml = () => {
    ...
    if (ext === "txt") {
      return `<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"></head>
        <body style="margin:16px;font-family:monospace;font-size:14px;color:#222;white-space:pre-wrap;word-break:break-word;">
        <script>try{document.body.innerText=decodeURIComponent(escape(atob("${base64Data}")));}catch(e){document.body.innerText="Could not decode file."}<\/script>
        </body></html>`;
    }
  ...
  const canRender = status === "ready" && (isImage ? !!localUri : base64Data.length > 50);
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
        <View style={dv.header}>
          ...
          <TouchableOpacity
            onPress={() => {
              const safeName = name.replace(/[^a-zA-Z0-9._-]/g, "_");
              const cacheKey = `${FileSystem.documentDirectory ?? ""}schoolaid_cache/${safeName}`;
              FileSystem.deleteAsync(cacheKey, { idempotent: true }).catch(() => {});
              setBase64(""); setLocalUri(""); setErrMsg(""); downloadAndRender();
            }}
            style={dv.reloadBtn}
          >
        ...
        <View style={{ flex: 1, backgroundColor: "#fff" }}>
          ...
          {status === "error" && (
            <View style={dv.overlay}>
              ...
              {!!errMsg && (
                <Text style={{ color: "rgba(255,255,255,0.55)", fontSize: 11, marginTop: 6, textAlign: "center", paddingHorizontal: 28 }}>
                  {errMsg}
                </Text>
              )}
              <TouchableOpacity onPress={downloadAndRender} style={dv.retryBtn}>
          ...
          {canRender && isImage && (
            <ScrollView maximumZoomScale={4} minimumZoomScale={1} centerContent
              contentContainerStyle={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
              <Image source={{ uri: localUri }} style={{ width: SCREEN_W, height: 400 }} resizeMode="contain" />
            </ScrollView>
          )}
          ...
          {canRender && isPdf && (
            <WebView
              ...
              mixedContentMode="always" scalesPageToFit startInLoadingState
              renderLoading={() => (
                <View style={[dv.overlay, { backgroundColor: "#fff" }]}>
                  <ActivityIndicator size="large" color={PRIMARY} />
                  <Text style={[dv.overlayText, { color: "#555" }]}>Rendering PDF…</Text>
                </View>
              )}
              onError={() => setStatus("error")}
            />
          ...
          {canRender && !isImage && !isPdf && (
            <WebView
              ...
              renderLoading={() => (
                <View style={[dv.overlay, { backgroundColor: "#fff" }]}>
                  <ActivityIndicator size="large" color={PRIMARY} />
                  <Text style={[dv.overlayText, { color: "#555" }]}>Rendering…</Text>
                </View>
              )}
              onError={() => setStatus("error")}
            />
...
function StatusTick({ 
  ...
  if (!status || status === "sending") {
    return <Ionicons name="time-outline" size={11} color="rgba(3, 3, 3, 0.5)" />;
  }
  
  if (status === "sent") {
    // Only show double tick for the last message if thread has been read (unreadCount === 0)
    if (isLastMessage && unreadCount === 0) {
      return <Ionicons name="checkmark-done-outline" size={11} color="#0a0a0a" />;
    }
    return <Ionicons name="checkmark-outline" size={11} color="rgba(12, 12, 12, 0.7)" />;
  }
...
export default function ChatScreen() {
  ...
  const loadPinned = async () => {
    try { const raw = await AsyncStorage.getItem(`pinnedMsgs_${childId}`); if (raw) setPinnedIds(JSON.parse(raw)); } catch {}
  };
  const loadSavedFiles = async () => {
    try { const raw = await AsyncStorage.getItem(DOWNLOADS_KEY); if (raw) setSavedFiles(JSON.parse(raw)); } catch {}
  };
  const persistSavedFile = async (entry: DownloadedFile) => {
    try { const raw = await AsyncStorage.getItem(DOWNLOADS_KEY); const current: DownloadedFile[] = raw ? JSON.parse(raw) : []; const updated = [entry, ...current.filter((f) => f.url !== entry.url)]; await AsyncStorage.setItem(DOWNLOADS_KEY, JSON.stringify(updated)); setSavedFiles(updated); } catch {}
  };
  const removeSavedFile = async (key: string) => {
    try { const raw = await AsyncStorage.getItem(DOWNLOADS_KEY); const current: DownloadedFile[] = raw ? JSON.parse(raw) : []; await AsyncStorage.setItem(DOWNLOADS_KEY, JSON.stringify(current.filter((f) => f.key !== key))); setSavedFiles(current.filter((f) => f.key !== key)); } catch {}
  };
  ...
  // ── Mark thread as read ───────────────────────────────────
  // Calls PUT /api/parent-notes/messages/read/:thread_id
  // This marks all messages in the thread as read on the server.
  // When the teacher sends a message and the parent opens the chat,
  // the server will update is_read = true → blue ticks appear.
const markThreadAsRead = useCallback(async (t: string, tid: number) => {
  try {
    const yearId = await AsyncStorage.getItem("selectedYearId") ?? "8";
    await fetch(`${BASE_URL}/api/parent-notes/messages/read/${tid}`, {
      method: "PUT",
      headers: {
    ...
    // After marking as read, update unread count to 0
    setUnreadCount(0);
    
    // Optional: Force refresh messages to update status display
    if (token && threadId) {
      await fetchMessages(t, tid);
    }
  } catch {
...
useFocusEffect(useCallback(() => {
  const init = async () => {
    ...
    if (!isNewChat && threadId) {
      await fetchMessages(t, threadId);
      await fetchThreadDetails(t, threadId); // Fetch unread count
      await markThreadAsRead(t, threadId); // Mark thread as read when opened
    } else {
      setLoading(false);
    }
    await loadPinned();
...
useEffect(() => {
  ...
  const interval = setInterval(async () => {
    try {
      // Check if thread is still unread
      await fetchThreadDetails(token, threadId);
      ...
      const res = await fetch(`${BASE_URL}/api/parent-notes/threads/${threadId}/messages`, {
        headers: { 
          ...
          "Content-Type": "application/json", 
          "x-academic-year-id": (await AsyncStorage.getItem("selectedYearId")) ?? "8" 
        },
      });
      if (!res.ok) return;
      const data = await res.json();
...
const fetchThreadDetails = async (t: string, tid: number) => {
  try {
    const yearId = await AsyncStorage.getItem("selectedYearId") ?? "8";
    const res = await fetch(`${BASE_URL}/api/parent-notes/threads/${tid}`, {
      headers: {
        Authorization: `Bearer ${t}`,
        "x-academic-year-id": yearId,
      },
  ...
  const closeAllViewers = useCallback(() => {
    setDocViewer(null); setPreviewVideo(null); setAudioPlayer(null); setPreviewItem(null);
  }, []);
  ...
  const downloadFile = async (url: string, name: string, type: "file" | "video") => {
    ...
    try {
      if (type === "video") {
        ...
        if (status !== "granted") { Alert.alert("Permission required", "Please allow media library access."); setDownloads((prev) => { const n = { ...prev }; delete n[key]; return n; }); return; }
      }
      const folder = `${FileSystem.documentDirectory}SchoolAid/`;
      await FileSystem.makeDirectoryAsync(folder, { intermediates: true });
      const safeFileName = `${Date.now()}_${name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
      ...
      if (type === "video") {
        ...
        Alert.alert("Downloaded!", `"${name}" saved to Downloads tab.`);
      }
    } catch (err: any) {
  ...
  const DownloadIcon = ({ url, name, type, tintColor }: { url: string; name: string; type: "file" | "video"; tintColor: string }) => {
    ...
    return (
      <TouchableOpacity onPress={() => downloadFile(url, name, type)} style={styles.downloadBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        <Ionicons name={state === "done" || alreadySaved ? "checkmark-circle" : "arrow-down-circle-outline"} size={22} color={state === "done" || alreadySaved ? "#22C55E" : tintColor} />
      </TouchableOpacity>
    );
  };
  ...
  const handleRefreshMedia = async () => {
    ...
    if (threadId) await fetchMessages(token, threadId);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) setAttachments((prev) => [...prev, ...result.assets.map((a) => ({ uri: a.uri, type: "image" as const, name: a.fileName ?? "image.jpg", mimeType: a.mimeType ?? "image/jpeg" }))]);
  };
  const pickFile = async () => {
    const result = await DocumentPicker.getDocumentAsync({ type: "*/*", copyToCacheDirectory: true, multiple: true });
    if (!result.canceled && result.assets.length > 0) setAttachments((prev) => [...prev, ...result.assets.map((a) => ({ uri: a.uri, type: "file" as const, name: a.name, mimeType: a.mimeType ?? "application/octet-stream" }))]);
  };
  const openCamera = async () => {
    if (!cameraPermission?.granted) { const { granted } = await requestCameraPermission(); if (!granted) { Alert.alert("Permission required", "Please allow camera access."); return; } }
    if (!micPermission?.granted) { const { granted } = await requestMicPermission(); if (!granted) { Alert.alert("Permission required", "Please allow microphone access."); return; } }
    setShowCamera(true);
  };
  const showAttachmentOptions = () => Alert.alert("Attach", "Choose attachment type", [{ text: "Photo", onPress: pickImage }, { text: "File", onPress: pickFile }, { text: "Video", onPress: openCamera }, { text: "Cancel", style: "cancel" }]);
  const removeAttachment = (idx: number) => setAttachments((prev) => prev.filter((_, i) => i !== idx));
  ...
  const handlePin = useCallback(async () => {
    const msg = contextMsgRef.current; if (!msg) return;
    const raw = await AsyncStorage.getItem(`pinnedMsgs_${childId}`); const ids: string[] = raw ? JSON.parse(raw) : [];
    const updated = ids.includes(msg.id) ? ids.filter((i) => i !== msg.id) : [...ids, msg.id];
    await AsyncStorage.setItem(`pinnedMsgs_${childId}`, JSON.stringify(updated)); setPinnedIds(updated); setContextMsg(null);
  }, [childId]);
  const handleDelete = useCallback(async () => {
    const msg = contextMsgRef.current; if (!msg) return; setContextMsg(null);
    Alert.alert("Delete Message", "Are you sure?", [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: async () => {
      try {
        const res = await fetch(`${BASE_URL}/api/parent-notes/messages/${msg.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "x-academic-year-id": (await AsyncStorage.getItem("selectedYearId")) ?? "16" } });
        if (res.ok) { setMessages((prev) => prev.filter((m) => m.id !== msg.id)); setPinnedIds((prev) => { const updated = prev.filter((id) => id !== msg.id); AsyncStorage.setItem(`pinnedMsgs_${childId}`, JSON.stringify(updated)); return updated; }); }
  ...
  const scrollToMessage = useCallback((msgId: string) => {
    setMessages((current) => { const idx = current.findIndex((m) => m.id === msgId); if (idx !== -1) flatListRef.current?.scrollToIndex({ index: idx, animated: true, viewPosition: 0.3 }); return current; });
  }, []);
  ...
  const sendMessage = async () => {
    ...
    const tempMsg: Message = {
      id: String(Date.now()), text: sentText, sender: "parent", createdAt: new Date().toISOString(),
      attachmentUrl: sentFiles[0]?.uri, attachmentType: sentFiles[0]?.type, attachmentName: sentFiles[0]?.name,
      replyToId: sentReplyTo?.id, replyToText: sentReplyTo?.text || sentReplyTo?.attachmentName,
      status: "sending",
    };
    setMessages((prev) => [...prev, tempMsg]);
    ...
    try {
      ...
      if (sentFiles.length > 0) {
        ...
        if (threadId) {
          ...
          res = await fetch(`${BASE_URL}/api/parent-notes/threads`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "x-academic-year-id": yearId }, body: JSON.stringify({ student_id: Number(childId), message: encodeURIComponent(sentText), ...(sentReplyTo ? { reply_to_id: sentReplyTo.id } : {}) }) });
          console.log("Thread creation response:", res);
        }
        if (!res.ok) { setMessages((prev) => prev.filter((m) => m.id !== tempMsg.id)); throw new Error(`HTTP ${res.status}`); }
        const data = JSON.parse(await res.text());
        const returned = data.thread_id ?? data.data?.thread_id;
  ...
  const filteredMessages = searchQuery.trim()
    ? messages.filter((m) => { const q = searchQuery.toLowerCase(); return m.text?.toLowerCase().includes(q) || m.attachmentName?.toLowerCase().includes(q) || formatDate(m.createdAt).toLowerCase().includes(q); })
    : messages;
      ...
      <View style={[styles.msgRow, isParent && styles.msgRowRight]}>
        ...
        <View style={{ maxWidth: "78%" }}>
          ...
          <View style={[styles.bubble, isParent ? styles.bubbleRight : styles.bubbleLeft, isPinned && styles.bubblePinned]}>
            ...
            <View style={[styles.timeRow, isParent && { justifyContent: "flex-end" }]}>
              <Text style={[styles.timeText, isParent && { color: "rgba(255,255,255,0.6)" }]}>{formatTime(msg.createdAt)}</Text>
              {isParent && <StatusTick status={msg.status} isLastMessage={isLastMessage} unreadCount={unreadCount} />}
            </View>
          </View>
        </View>
      </View>
  ...
  const renderDownloads = () => {
    ...
    const openDownloadedFile = async (file: DownloadedFile) => {
      try {
        ...
        if (!info.exists) { Alert.alert("File not found", "This file was cleared from device storage.", [{ text: "Remove from list", onPress: () => removeSavedFile(file.key), style: "destructive" }, { text: "OK", style: "cancel" }]); return; }
        const n = (file.name ?? "").toLowerCase(); const ext = n.split(".").pop() ?? "";
        const isVideoFile = ext === "mp4" || file.type === "video" || n.includes("video_");
        const isVoiceFile = ext === "m4a" || n.includes("voice_");
        const isImageFile = ["jpg","jpeg","png","gif","webp"].includes(ext);
        closeAllViewers(); await new Promise<void>((resolve) => setTimeout(resolve, 120));
        if (isVideoFile) { setPreviewVideo(file.localUri); return; }
        if (isVoiceFile) { setAudioPlayer({ uri: file.localUri, name: file.name }); return; }
        if (isImageFile) { setPreviewItem({ uri: file.localUri, type: "image", name: file.name }); return; }
        setDocViewer({ uri: file.localUri, name: file.name });
      } catch (err: any) { Alert.alert("Error", `Could not open file: ${err?.message ?? "unknown error"}`); }
    };
    const deleteFile = (file: DownloadedFile) => Alert.alert("Remove Download", `Remove "${file.name}"?`, [{ text: "Cancel", style: "cancel" }, { text: "Remove", style: "destructive", onPress: async () => { try { const info = await FileSystem.getInfoAsync(file.localUri); if (info.exists) await FileSystem.deleteAsync(file.localUri, { idempotent: true }); } catch {} await removeSavedFile(file.key); } }]);
    const refreshControl = <RefreshControl refreshing={refreshingDownloads} onRefresh={handleRefreshDownloads} colors={[PRIMARY]} tintColor={PRIMARY} />;
    if (savedFiles.length === 0) return <ScrollView contentContainerStyle={{ flexGrow: 1 }} refreshControl={refreshControl}><View style={[styles.emptyGallery, { marginTop: 60 }]}><View style={styles.dlEmptyIconWrap}><Ionicons name="cloud-download-outline" size={40} color={PRIMARY} /></View><Text style={styles.emptyGalleryTitle}>No downloads yet</Text><Text style={styles.emptyGalleryText}>Files and videos you download from chat will appear here.</Text></View></ScrollView>;
    ...
    <SafeAreaView style={styles.safe}>
        ...
        {(["chat", "media", "downloads"] as TabType[]).map((tab) => (
          <TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]} onPress={() => setActiveTab(tab)}>
            ...
            <Text style={[styles.tabText, activeTab === tab && styles.tabTextActive]}>
              {tab === "chat" ? "Chat" : tab === "media" ? `Media${mediaMessages.length + fileMessages.length + voiceMessages.length > 0 ? ` (${mediaMessages.length + fileMessages.length + voiceMessages.length})` : ""}` : `Downloads${savedFiles.length > 0 ? ` (${savedFiles.length})` : ""}`}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {previewItem?.type === "image" && (
        <Modal visible animationType="slide" transparent onRequestClose={() => setPreviewItem(null)}>
          <View style={styles.previewModal}><TouchableOpacity style={styles.previewClose} onPress={() => setPreviewItem(null)}><Ionicons name="close" size={28} color="#fff" /></TouchableOpacity><Image source={{ uri: previewItem.uri }} style={styles.fullImage} resizeMode="contain" /></View>
        </Modal>
      )}
