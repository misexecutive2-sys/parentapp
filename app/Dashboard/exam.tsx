...
export default function ExamResultsScreen() {
  ...
  // ── TODO: Replace with real download API when developer provides it ──
  const handleDownload = async () => {
    if (!selectedExamId || !studentId) {
      Alert.alert("Error", "Please select an exam first.");
      return;
    ...
    try {
      ...
      // TODO: Swap this URL with the real result-download API endpoint
      const res = await fetch(
        "https://connect.schoolaid.in/api/app/result-download", // ← replace with real endpoint
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (data.pdf_url) {
        // ✅ Opens the PDF URL in the device browser / PDF viewer
        await Linking.openURL(data.pdf_url);
      } else {
        // Placeholder alert until API is ready
        Alert.alert(
          "Coming Soon",
          "Result download will be available once the API is ready.",
          [{ text: "OK" }]
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={['top']}>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={styles.filters}>
            {exams.length > 0 ? (
              <>
                ...
                <View
                  style={[styles.pickerWrapper, { borderColor: theme.primary }]}
                >
                  <Picker
                    selectedValue={selectedExamId}
          ...
          {selectedExam && (
            <View
              style={[styles.examInfoCard, { borderLeftColor: theme.primary }]}
            >
              <View style={{ flex: 1 }}>
                <Text style={[styles.examInfoName, { color: theme.primary }]}>
                  {selectedExam.exam_name ?? selectedExam.name}
